...
interface Table9Props {
  columns: MRT_ColumnDef<CycleData>[]; // คอลัมน์ที่จะแสดงในตาราง
  initialData: CycleData[]; // ข้อมูลเริ่มต้นที่จะแสดงในตาราง
}
...
const Table9 = memo(function Table9({ columns, initialData }: Table9Props) {
  //!----------------table state------------------!//
  ...
  // ฟังก์ชันจัดการการสร้างข้อมูลใหม่
  const handleCreate: MRT_TableOptions<CycleData>["onCreatingRowSave"] = async ({
    values,
    table,
  }) => {
    const newData = {
  ...
  const handleUpdate: MRT_TableOptions<CycleData>["onEditingRowSave"] = async ({
    values,
    table,
    row,
  }) => {
          ...
          >
            {internalEditComponents.map((component) => {
              const columnDef = (component as any).props.cell.column
                .columnDef as MRT_ColumnDef<CycleData>;
              if (columnDef.meta === "date") {
                // สร้างตัวแปร day เพื่อเก็บ accessorKey
                const day = columnDef.accessorKey as keyof CycleData;
                return (
