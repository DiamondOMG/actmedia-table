// src/app/components/table/Table6.tsx
...
interface Table6Props {
  columns: MRT_ColumnDef<SequenceData>[]; // คอลัมน์ที่จะแสดงในตาราง
  initialData: SequenceData[]; // ข้อมูลเริ่มต้นที่จะแสดงในตาราง
}
...
const Table6 = memo(function Table6({ columns, initialData }: Table6Props) {
  //!----------------table state------------------!//
  ...
  // ฟังก์ชันจัดการการสร้างข้อมูลใหม่
  const handleCreate: MRT_TableOptions<SequenceData>["onCreatingRowSave"] = async ({
    values,
    table,
  }) => {
    const newData = {
  ...
  const handleUpdate: MRT_TableOptions<SequenceData>["onEditingRowSave"] = async ({
    values,
    table,
    row,
  }) => {
          ...
          >
            {internalEditComponents.map((component) => {
              const columnDef = (component as any).props.cell.column
                .columnDef as MRT_ColumnDef<SequenceData>;
              if (columnDef.meta === "date") {
                // สร้างตัวแปร day เพื่อเก็บ accessorKey
                const day = columnDef.accessorKey as keyof SequenceData;
                return (
