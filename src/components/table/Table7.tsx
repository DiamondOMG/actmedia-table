...
interface Table7Props {
  columns: MRT_ColumnDef<CustomersData>[]; // คอลัมน์ที่จะแสดงในตาราง
  initialData: CustomersData[]; // ข้อมูลเริ่มต้นที่จะแสดงในตาราง
}
...
const Table7 = memo(function Table7({ columns, initialData }: Table7Props) {
  //!----------------table state------------------!//
  ...
  // ฟังก์ชันจัดการการสร้างข้อมูลใหม่
  const handleCreate: MRT_TableOptions<CustomersData>["onCreatingRowSave"] = async ({
    values,
    table,
  }) => {
    const newData = {
  ...
  const handleUpdate: MRT_TableOptions<CustomersData>["onEditingRowSave"] = async ({
    values,
    table,
    row,
  }) => {
          ...
          >
            {internalEditComponents.map((component) => {
              const columnDef = (component as any).props.cell.column
                .columnDef as MRT_ColumnDef<CustomersData>;
              if (columnDef.meta === "date") {
                // สร้างตัวแปร day เพื่อเก็บ accessorKey
                const day = columnDef.accessorKey as keyof CustomersData;
                return (
